...
function expectPrivateFile(filePath: string): void {
  if (supportsPosixModes) expect(fs.statSync(filePath).mode & 0o777).toBe(0o600);
}

function expectPrivateDirectory(dirPath: string): void {
...
describe('Node dashboard security', () => {
  ...
  it('creates the event log and containing directory with private permissions', async () => {
    ...
    expectPrivateFile(eventsFile);
    expectPrivateDirectory(path.dirname(eventsFile));
  });
  ...
  it('creates rendered PNG dumps with private permissions', async () => {
    ...
    expectPrivateDirectory(dumpDir);
    expect(files.length).toBeGreaterThan(0);
    expectPrivateFile(path.join(dumpDir, files[0]!));
